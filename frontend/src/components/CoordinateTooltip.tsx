  ...
  return (
    <>
      <div className="tooltip-section">
        <span className="tooltip-label">גובה טיסה מפני הים:</span> {(point.elevation + (point.flightHeight ?? 0)).toFixed(1)} מ'
      </div>
      {point.minElevation !== undefined && (
        <div className="tooltip-section">
          <span className="tooltip-label">גובה טיסה ממינימום:</span> {((point.elevation + (point.flightHeight || 0)) - point.minElevation).toFixed(1)} מ'
        </div>
      )}
      {point.maxElevation !== undefined && (
        <div className="tooltip-section">
          <span className="tooltip-label">גובה טיסה ממקסימום:</span> {((point.elevation + (point.flightHeight || 0)) - point.maxElevation).toFixed(1)} מ'
        </div>
      )}
      {point.minElevation !== undefined && (
        <div className="tooltip-section">
